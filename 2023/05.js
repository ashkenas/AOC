...
const seeds = input.shift().split(': ')[1].split(' ').map((n) => +n);
const seedPairs = [];
...
const makeMap = (mapText) => {
  ...
  const solver = (ranges) => {
    const converters = rows.filter(([, src, range]) =>
    ...
    ranges.forEach(([startA, endA]) => {
      ...
      for (const [dest, startB, range] of converters) {
        ...
        res.push([interStart - startB + dest, interEnd - startB + dest])
        startA = interEnd + 1;
        if (endA <= endB) break;
      }
      if (startA <= endA)
...
const reduceMaps = (initial) => Math.min(...maps.reduce(
  (val, curr) => curr(val),
  initial
).map(([start]) => start));
